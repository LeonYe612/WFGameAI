...
import { TaskType } from "@/views/tasks/utils/enums";
import { runTypeConfig } from "@/views/tasks/utils/rules";
...
export const useScriptListTable = (props?: any) => {
    ...
    const handleExecute = async (_row, run_type: number) => {
        const queryParams = {
            name: `${runTypeConfig?.[run_type]?.label ?? String(run_type)}_${Date.now()}`,
            script_ids: [_row.id],
            task_type: TaskType.Replay, // 回放类型 - todo: 后续可能会有其他任务类型
