...
def validate_test_result(cmd, desc, result, device_id):
    ...
    if "基本shell命令" in desc:
        ...
        time.sleep(2)
        check_result = subprocess.run(f"adb -s {device_id} shell dumpsys window | findstr \"mCurrentFocus\"",
                                    shell=True, capture_output=True, text=True, timeout=5, encoding='utf-8', errors='ignore')        if check_result.returncode == 0:
            focus = check_result.stdout.strip()
        ...
        if result.returncode == 0:
            ...
            if success_rate >= 0.25:  # 至少25%成功即视为滑动功能正常
                success = True
...
def test_adb_commands(device_id):
    ...
    tests = [
        ...
        # 修改截图命令，使用adb命令完整路径
        (f"shell screencap /sdcard/screenshot.png", "屏幕截图功能"),
