...
class OCRGitRepository(models.Model):
    """OCR Git仓库模型"""
    project = models.ForeignKey(OCRProject, on_delete=models.CASCADE, related_name='repositories', verbose_name='所属项目')
...
class OCRTask(models.Model):
    ...
    match_rate = models.DecimalField(max_digits=5, decimal_places=2, default=0.00, verbose_name='匹配率')
    created_at = models.DateTimeField(auto_now_add=True, verbose_name='创建时间')
...
class OCRResult(models.Model):
    """OCR结果模型"""
    task = models.ForeignKey(OCRTask, on_delete=models.CASCADE, related_name='results', verbose_name='所属任务')
    ...
    processing_time = models.DecimalField(max_digits=10, decimal_places=3, null=True, blank=True, verbose_name='处理时间')
    created_at = models.DateTimeField(auto_now_add=True, verbose_name='创建时间')
