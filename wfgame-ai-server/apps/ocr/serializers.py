...
class OCRGitRepositorySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'project', 'url', 'branch', 'created_at']


class OCRTaskSerializer(serializers.ModelSerializer):
    """OCR任务序列化器"""
    project_name = serializers.ReadOnlyField(source='project.name')
    git_repository_url = serializers.ReadOnlyField(source='git_repository.url', default='')
...
class OCRResultSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = OCRResult
        fields = ['id', 'task', 'task_id', 'image_path', 'texts', 'languages',
...
class TaskCreationSerializer(serializers.Serializer):
    ...
    image_formats = serializers.ListField(child=serializers.CharField(), required=False,
                                         default=['jpg', 'jpeg', 'png', 'bmp', 'tiff', 'tif', 'webp'])
    target_languages = serializers.ListField(child=serializers.CharField(), required=False,
                                           default=['ch'])
    use_gpu = serializers.BooleanField(required=False, default=True)
    gpu_id = serializers.IntegerField(required=False, default=0)
...
class FileUploadSerializer(serializers.Serializer):
    ...
    def validate_file(self, value):
        ...
        # 验证文件扩展名
        allowed_extensions = ['.jpg', '.jpeg', '.png', '.bmp', '.tiff', '.tif', '.webp', '.zip', '.tar.gz', '.tgz']
        ext = value.name.lower()
        if not any(ext.endswith(allowed_ext) for allowed_ext in allowed_extensions):
            raise serializers.ValidationError(f"不支持的文件类型，允许的格式: {', '.join(allowed_extensions)}")
