...
class AppPermissionManager:
    ...
    def __init__(self, device_id: str = None, config: Optional[Dict[str, Any]] = None):
        """
        初始化权限管理器
    ...
    def _detect_app_custom_permission_dialog(self, root) -> Optional[PermissionDialog]:
        ...
        try:
            ...
            is_actual_login_screen = (
                ...
                not any(action_text in combined_text for action_text in ["同意", "不同意", "允许", "拒绝", "Accept", "Deny"])  # 没有权限相关按钮
            )
    ...
    def _click_permission_button(self, device_serial: str, action: PermissionAction) -> bool:
        """
        点击权限弹窗中的指定按钮 - 精简版：使用3种成功验证的点击方法
        ...
        每种方法点击后检查元素是否仍存在，如存在则尝试下一种方法
        """
        try:
            # 获取点击前的UI状态，用于验证点击是否成功
            ui_before = self._get_ui_dump(device_serial)
            if not ui_before:
                logger.warning("无法获取点击前的UI状态")
                return False

            # 获取目标按钮的文本和resource_id模式
            ...
            logger.info(f"查找目标操作 {action.value} 的按钮")
            logger.info(f"文本模式: {target_texts}")
            logger.info(f"Resource ID模式: {target_resource_ids}")

            # 方法1: 文本匹配点击（第一优先级）
            if self._click_by_text_match(device_serial, target_texts):
                # 检查点击是否成功（元素是否消失）
                if self._element_disappeared(device_serial, ui_before, target_texts, target_resource_ids):
                    logger.info(f"✅ 通过文本匹配成功点击按钮")
                    return True
                logger.warning("文本匹配点击后元素仍存在，尝试下一种方法")

            # 方法2: Resource ID点击（第二优先级）
            if self._click_by_resource_id_simple(device_serial, target_resource_ids):
                # 检查点击是否成功
                if self._element_disappeared(device_serial, ui_before, target_texts, target_resource_ids):
                    logger.info(f"✅ 通过Resource ID成功点击按钮")
                    return True
                logger.warning("Resource ID点击后元素仍存在，尝试下一种方法")

            # 方法3: 坐标点击（第三优先级）
            if self._click_by_coordinates(device_serial, target_texts, target_resource_ids):
                # 检查点击是否成功
                if self._element_disappeared(device_serial, ui_before, target_texts, target_resource_ids):
                    logger.info(f"✅ 通过坐标点击成功点击按钮")
                    return True
                logger.warning("坐标点击后元素仍存在")

            logger.warning(f"❌ 所有3种点击方式都失败了: {action.value}")
            return False
    ...
    def _click_by_text_selector(self, device_serial: str, button_text: str) -> bool:
        """方式1: 通过UiAutomator文本选择器点击 - 优化版，参考enhanced_input_handler成功做法"""
        try:
            logger.info(f"🎯 尝试通过文本选择器点击: '{button_text}'")

            # 方法1: 使用UI dump + 直接坐标点击 (参考enhanced_input_handler的成功做法)
            ui_dump = self._get_ui_dump(device_serial)
            if ui_dump:
                # 在UI dump中查找文本匹配的可点击元素
                import xml.etree.ElementTree as ET
                root = ET.fromstring(ui_dump)

                for element in root.iter():
                    element_text = element.get('text', '').strip()
                    if element_text == button_text and element.get('clickable') == 'true':
                        bounds = element.get('bounds', '')
                        if bounds:
                            # 使用enhanced_input_handler的坐标解析方法
                            coords = self._parse_bounds_enhanced(bounds)
                            if coords:
                                center_x, center_y = coords
                                logger.info(f"📍 找到目标按钮，点击坐标: ({center_x}, {center_y})")

                                # 使用统一的adb点击命令格式
                                success = self._execute_adb_tap(device_serial, center_x, center_y)
                                if success:
                                    logger.info(f"✅ 文本选择器点击成功: '{button_text}'")
                                    return True

            # 方法2: 使用回车键确认（备选方法）
            try:
                if button_text in ["确定", "OK", "Allow", "允许", "同意"]:
                    cmd = f'adb -s {device_serial} shell "input keyevent KEYCODE_ENTER"'
            ...
            logger.debug(f"text selector点击失败: {e}")
            return False

    def _click_by_resource_id(self, device_serial: str, action: PermissionAction) -> bool:
        """方式2: 通过Resource ID点击"""
        try:
            # 根据操作类型确定可能的Resource ID
            possible_ids = []
            ...
            for resource_id in possible_ids:
                try:
                    cmd = f'adb -s {device_serial} shell "uiautomator runtest UIAutomatorStub.jar -c com.github.uiautomatorstub.Stub -e resourceId \'{resource_id}\' -e action click"'
    ...
    def _click_by_content_desc(self, device_serial: str, button_text: str) -> bool:
        """方式3: 通过Content Description点击"""
        try:
            cmd = f'adb -s {device_serial} shell "uiautomator runtest UIAutomatorStub.jar -c com.github.uiautomatorstub.Stub -e contentDescription \'{button_text}\' -e action click"'
            result = subprocess.run(cmd, shell=True, capture_output=True, text=True, timeout=5)
            ...
            logger.debug(f"Content Description点击失败: {e}")
            return False
